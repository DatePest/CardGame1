...
public class GameNotifyAction_Net  : NetworkBehaviour
{
    ...
    {
        var G = CardGameManager.Instance.CardSpawnManager.CardSpawnScripts[(int)clientId].FindCard(CardId, From);
        if (G == null) return;
    ...
    void CardUseEndNotifyClientRpc(int CardId, ulong clientId, CardsPileEnum From, CardsPileEnum Target)
    {
        ...
        var G = CardGameManager.Instance.CardSpawnManager.CardSpawnScripts[(int)clientId].FindCard(CardId, From);
        if (G == null) return;
        CardGameManager.Instance.CardSpawnManager.CardSpawnScripts[(int)clientId].CardsPileChange(G, From, Target);
    }
    ...
    {
        if (NetworkManager.Singleton.LocalClientId == clientId) return;
        var G = CardGameManager.Instance.CardSpawnManager.CardSpawnScripts[(int)clientId].FindCardAll(UseCardUid);
    ...
    {
        if (NetworkManager.Singleton.LocalClientId == clientId) return;
        var G = CardGameManager.Instance.CardSpawnManager.CardSpawnScripts[(int)clientId].FindCardAll(NetData.UseCardUid);
        var so = G.GetComponentInChildren<CardSolt>();
     ...
     void UseCheckDisCardClientRpc(ulong clientId, Net_AbilityNeedData NetData, CardsPileEnum From, CardsPileEnum Target)
    {
        ...
        CardGameManager.Instance.CardSpawnManager.CardSpawnScripts[(int)clientId].FindCardGoto(data.SelectCardUids, From, Target);
    }
    [ServerRpc]
