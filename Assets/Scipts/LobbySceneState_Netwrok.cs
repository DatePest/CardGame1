...
{
    public List<LobbyPlayerState> m_LobbyPlayers;
    ...
    private void LobbyListChanged()
    {
        ...
        for (int i = 0; i < lobbyManager._Players.Length; i++)
        {
    ...
    void lobbySet_ClientRpc(int i, LobbyPlayerState State)
    {
        ...
        lobbyManager._Players[i].SetOwner(State.ClientId, State.m_PlayerName);
        if (State.Ready)
            lobbyManager._Players[i].SetIsReady(State.Ready);
    }
    [ClientRpc]
    ...
    {
        lobbyManager._Players[State.Number].SetIsReady(State.Ready);
    }
