    ...
    protected Unit SetSortTpye(List<Unit> LastList, UnitSortSelectType selectType, CheckSortTpye SortTpye)
    {
        switch (SortTpye)
        {
            ...
            case CheckSortTpye.Armor:
                if (selectType == UnitSortSelectType.Max)
                    return LastList.Where(n => n.UnitData.LastArmor == LastList.Max(n => n.UnitData.ArmorAtk)).FirstOrDefault();
                else
                    return LastList.Where(n => n.UnitData.LastArmor == LastList.Min(n => n.UnitData.ArmorAtk)).FirstOrDefault();
        }
